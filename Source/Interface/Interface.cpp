    ...
    for (int x = 0; x < playerHotBar.getSize().x; x++) {
        ...
        if (cell.amount) {
            ...
            tgui::Texture texture{resourceManager->getTexture(ResourceManager::ITEMS),
                                  {static_cast<unsigned int>(textRect.left), static_cast<unsigned int>(textRect.top),
                                   static_cast<unsigned int>(textRect.width), static_cast<unsigned int>(textRect.height)}};
    ...
    for (int x = 0; x < playerBackpack.getSize().x; x++) {
        for (int y = 0; y < playerBackpack.getSize().y; y++) {
            ...
            if (cell.amount) {
                ...
                tgui::Texture texture{resourceManager->getTexture(ResourceManager::ITEMS),
                                      {static_cast<unsigned int>(blockRect.left), static_cast<unsigned int>(blockRect.top),
                                       static_cast<unsigned int>(blockRect.width), static_cast<unsigned int>(blockRect.height)}};
                picture->getRenderer()->setTexture(texture);
                picture->setOrigin(0.5f, 0.5f);
                picture->setPosition({"parent.parent.inventory.cells.cell" + std::to_string(x) + std::to_string(y)
                                          + ".position + parent.parent.inventory.cells.cell" + std::to_string(x) + std::to_string(y) + ".size / 2"});
...
void Interface::highlightHotBarCell(const Player& player)
{
    ...
    tgui::Group::Ptr hotBar{gui_.get<tgui::Group>("hotBar")};
    hotBar->get<tgui::Picture>("cell" + std::to_string(previousHighlightedCell))->getRenderer()
        ->setTexture({resourceManager->getTexture(ResourceManager::INVENTORY_CELL), {0, 0, 31, 31}});
    hotBar->get<tgui::Picture>("cell" + std::to_string(player.getHotBarIndex()))->getRenderer()
        ->setTexture({resourceManager->getTexture(ResourceManager::INVENTORY_CELL), {31, 0, 31, 31}});
