    ...
    window_{ window }
{
    ...
    hotBar->add(hotBarPicture, "hotBarPicture");
    for (int i = 0; i < 10; i++) {
        ...
        cell->setPosition({ "parent.hotBarPicture.position + 26 + (size + 4) * " + std::to_string(i) },
                          "parent.hotBarPicture.position + 22");
...
void Interface::updateHealth(const Player &player) {
    ...
    for (int i = 0; i < fullHearts; i++) {
        ...
        heart->setPosition({"parent.parent.hotBar.hotBarPicture.position + 35 + (22.0f + 8.0f) * "  + std::to_string(i)},
                           "parent.parent.hotBar.hotBarPicture.position - 13.0f");
